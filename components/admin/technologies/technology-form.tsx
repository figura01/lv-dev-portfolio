...
import { technologiyDefaultValue } from "@/lib/constantes";
...
const TechnologyForm = ({
  ...
  type: "create" | "update";
  technology?: z.infer<typeof technologySchema>;
  technologyId?: string;
}) => {
  ...
  type FormValues = z.infer<typeof insertTechnologySchema>;

  const form = useForm<FormValues>({
    resolver: zodResolver(
      isUpdate ? updateTechnologySchema : insertTechnologySchema
    ),
    defaultValues: (technology && isUpdate
      ? { ...technology }
      : technologiyDefaultValue) as FormValues,
  });

  const onSubmit = async (values: z.infer<typeof technologySchema>) => {
    setIsLoading(true);
    try {
      const existedTechnology = await getTechnologyByName(values.name);
      if (existedTechnology) {
        toast.error("Une technologie avec ce nom existe déjà");
        return;
      }
      if (!isUpdate) {
